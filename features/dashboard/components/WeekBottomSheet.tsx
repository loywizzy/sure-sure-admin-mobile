    ...
    >
      <BottomSheetView className="flex-1 px-6">
        <View className="mb-5 items-center pt-1">
          <Text className="text-2xl font-extrabold text-gray-900">เลือกสัปดาห์</Text>
          <Text className="mt-1 text-base text-gray-500">สัปดาห์เริ่มวันจันทร์–อาทิตย์</Text>
        </View>
        ...
        <View className="mb-4 flex-row justify-center">
          ...
          <TouchableOpacity
            className="mx-1 rounded-full border border-gray-200 bg-white px-4 py-2"
            onPress={() => {
          ...
          >
            <Text className="text-gray-700">สัปดาห์ที่แล้ว</Text>
          </TouchableOpacity>
        </View>
        ...
        {weeks.map((w, idx) => {
          ...
          return (
            <TouchableOpacity
              key={idx}
              className={`mb-3 rounded-xl border px-4 py-3 ${isCurrent ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'}`}
              onPress={() => onSelect(iso)}
              activeOpacity={0.8}
            >
              <View className="flex-row items-center justify-between">
                <Text className={`text-base ${isCurrent ? 'text-blue-700' : 'text-gray-800'}`}>{formatRange(w)}</Text>

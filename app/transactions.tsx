...
export default function TransactionsScreen() {
  ...
  const renderCard = (t: DisplayTxn, idx: number) => {
    ...
    return (
      <View
        key={idx}
        className="mb-4 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4"
        style={{
          shadowColor: tone === 'success' ? '#22c55e' : '#ef4444',
        ...
        <Text className="mb-2 text-lg font-extrabold tracking-wider text-gray-900 dark:text-gray-100">#{t.id}</Text>
        <Text className="text-sm text-gray-700 dark:text-gray-300">| รหัสลูกค้า: {t.customerNo}</Text>
        <Text className="text-sm text-gray-700 dark:text-gray-300">| ชื่อลูกค้า: {t.firstName} {t.lastName}</Text>
        <Text className="text-sm text-gray-700 dark:text-gray-300">| ธนาคาร: {t.bank}</Text>
        <Text className="text-sm text-gray-700 dark:text-gray-300">| วันที่ทำรายการ: {t.createdAt.toLocaleString('th-TH', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' })}</Text>
        <Text className="text-sm text-gray-700 dark:text-gray-300">| ErrorMsg: {t.errorMsg ?? '-'}</Text>
        <Text className="mb-3 text-sm text-gray-700 dark:text-gray-300">| Transfer ID: {t.transferId}</Text>
    ...
    <View className="flex-1 bg-gray-50 dark:bg-gray-950">
      ...
      <ScrollView className="flex-1 px-4 pt-3">
        ...
        <View className="mb-4 rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-5 shadow-md">
          ...
          <View className="mb-3 flex-row items-center">
            ...
            <TouchableOpacity
              ...
              <Text className={`text-sm ${startDate ? 'text-gray-800 dark:text-gray-200' : 'text-gray-400'}`}>{startDate ? formatDate(startDate) : 'วันเริ่มต้น'}</Text>
            ...
            <TouchableOpacity
              ...
              <Text className={`text-sm ${endDate ? 'text-gray-800 dark:text-gray-200' : 'text-gray-400'}`}>{endDate ? formatDate(endDate) : 'วันสิ้นสุด'}</Text>
              <Text className="text-gray-500 dark:text-gray-400">📅</Text>
            </TouchableOpacity>
            {/* Clear */}
            <TouchableOpacity
          ...
          <View className="flex-row items-center">
            ...
            <TouchableOpacity
              ...
              <Text className={`text-sm ${selectedStatus ? 'text-gray-900 dark:text-gray-100' : 'text-gray-400'}`}>{selectedStatus || 'ทั้งหมด'}</Text>
              <Text className="text-gray-500 dark:text-gray-400">▾</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        {(searchName || selectedStatus || startDate || endDate) && (
          <View className="mb-4 rounded-xl bg-blue-50 dark:bg-blue-900/30 px-4 py-3">
